# network_automation/cisco/novenoadmredes/practica3/pnetlab/configDisp/r1.py

from netmiko import ConnectHandler

from network_automation.cisco.novenoadmredes.practica3.pnetlab.devices import R1, SYSLOG_SERVER_IP
...
def configurar(conn):
    # Interfaces e0/0 y e0/1
    conn.send_config_set([
        "interface ethernet0/0",
        " ip address 192.168.241.100 255.255.255.0",
        " no shutdown",
    ...
    # Syslog centralizado hacia Windows (tftpd64)
    conn.send_config_set([
    ...
    # EEM Applet: detecta interfaz Ethernet que cae
    conn.send_config_set([
        "event manager applet INTERFAZ_CAIDA",
        ' event syslog pattern ".*UPDOWN.*Ethernet.*down.*"',
        ' action 1.0 syslog priority critical msg "ALERTA: Interfaz Ethernet caida detectada"',
        "exit",
    ])

    # EEM Applet: detecta login exitoso por Telnet/SSH
    conn.send_config_set([
    ...
    # EEM Applet: heartbeat cada 5 minutos para confirmar que el sistema esta activo
    conn.send_config_set([
...
def main():
    print("Configurando R1 (192.168.241.100)...")
    try:
